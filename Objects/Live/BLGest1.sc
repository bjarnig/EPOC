BLGest1 : BSeq
{
	...
	*new { |id=0, description="BLGest1", duration=10, control, outBus=0, durations, sound, startPos=0, jitAmt=0.0, jitFreq=1, load=1|
	...
	init {|load=1|
		...
		reverb = Bwrap.new(\bLGest1Reso, [\inBus, effectBus, \outBus, outBus]);
		reverb.play; });
	}
	...
	*loadSynthDefs {
		...
		SynthDef(\bLGest1Reso, {| outBus = 0, inBus=2, amp=1, dryWet=0.01, roomsize=20, revtime=5.85, damping=0.81, inputbw=0.19,
	...
	update {
		...
		rate = Env.new([0.125, 0.75, 1, 1.5, 8],[0.25, 0.25, 0.25, 0.25]).at(control.frequency);
