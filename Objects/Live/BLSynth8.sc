...
BLSynth8 : BGen
{
	...
	*new { |id=0, description, duration=10, control, outBus=0, values, durations, amplitudes, noiseType=1, load=1|
	...
	initEffect {
		...
		reverb = Bwrap.new(\bLSynth8Verb, [\in, effectBus, \out, outBus]);
		reverb.play;
	}
	...
	*loadSynthDefs {
		...
		brownAmt=1.0, whiteAmt=1.0, pinkAmt=1.0, dustAmt=1.0, crackleAmt=1.0, durations=#[0.5, 1, 0.5, 2, 4, 0.5, 1], amplitudes=#[0.1, 1.0, 0.4, 0.8, 1.0, 0.1, 0.7, 0.1]|
	 	var env = EnvGen.kr(Env.new([0,1,1,0], [atk,sus,rel], -2), doneAction:2);
	 	var signal = (Crackle.ar(crackleSpeed, crackleAmt) + Dust.ar(dustSpeed, mul:(dustAmt*1.5)));
	 	var mod = (signal * LFNoise1.ar(32 * speed) * entropy);
		signal = (signal.tanh * (1 - entropy)) + mod;
		...
		SynthDef(\bLSynth8Verb,{| in = 3, out = 0, amp=1.0, mix=0.5, decay=1.0, density=0.25, fund=60, randMin=1.0, randMax=1.0|
Ê Ê Ê Ê Ê Ê Ê Ê Ê Ê Ê Ê Ê Ê Ê Ê Ê Ê Ê Ê
Ê Ê Ê	var input	, combi1, combi2, output, reso, effect;
		input = In.ar(in, 2);
		reso = input;
...
Ê Ê Ê Ê	Out.ar(out, output * amp);
	...
	if(updateAll == 1 || (control.position != preControl.position),
	{
		...
		wrap.set(\dustSpeed, Env.new([0.1, 25, 5, 75, 100],[0.25, 0.25, 0.25, 0.25]).at(control.position));
		wrap.set(\crackleSpeed, Env.new([1.4, 1.55, 1.72, 1.82, 1.99],[0.25, 0.25, 0.25, 0.25]).at(control.position));
		// wrap.set(\brownAmt, Env.new([1.8, 1.6, 1.4, 1.2],[0.2, 0.2, 0.2]).at(control.position));
		// wrap.set(\whiteAmt, Env.new([1.2, 1.4, 1.6, 1.8],[0.2, 0.2, 0.2]).at(control.position));
	});
