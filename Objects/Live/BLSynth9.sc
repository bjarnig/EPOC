...
BLSynth9 : BGen
{
	...
	*new { |id=0, description, duration=10, control, outBus=0, values, durations, amplitudes, osc=0, curveType=0, load=1|
	...
	initEffect {
		...
		reverb = Bwrap.new(\bLSynth9Verb, [\inBus, effectBus, \outBus, outBus]);
		reverb.play;
	}

	*loadSynthDefs {

		SynthDef(\bLSynth9, {arg  outBus=0, amp=0.5, atk=0.5, sus=10, rel=0.1, freq=1000, pan=0.8, entropy=0.5, entropySpeed=0.5,
		...
		(LinCongC.ar(freq, m:0.1) + LinCongC.ar(freq * 0.25, m:0.1) + LinCongC.ar(freq * 4.0, m:0.1) * 0.1) * amp2,
		(QuadL.ar(freq*0.125) + QuadL.ar(freq * 0.5) + QuadL.ar(freq * 0.0625)) * amp1]);
