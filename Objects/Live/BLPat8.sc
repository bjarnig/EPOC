...
BLPat8 : BGen
{
	...
	*new { |id=0, description, duration=10, control, outBus=0, values, sounds, path, load=1|
	...
	initEffect {
		...
		reverb = Bwrap.new(\bLPat8Verb, [\in, effectBus, \out, outBus]);
		reverb.play;
	}
	...
	*loadSynthDefs {
		...
		SynthDef(\bLPat8Verb,{| in = 0, out = 0, amp=1.0, mix=0.5, decay=0.75, density=0.25, fund=60, randMin=0.8, randMax=1.2|
...
Ê Ê Ê Ê	Out.ar(out, output * amp);
	...
	setDescription {
		description = "BLPat8: Audio rate granulator.";
	}
}