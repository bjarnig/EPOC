...
BLPat5 : BGen
{
	...
	*new { |id=0, description, duration=10, control, outBus=0, values, sounds, path, load=1|
	...
	initEffect {
		...
		reverb = Bwrap.new(\bLPat5Verb, [\in, effectBus, \out, outBus]);
 		reverb.play;
	}
	...
	*loadSynthDefs {
		...
		trate = (speed * (SinOsc.ar(0.2, mul:0.5, add:1.0) * 15)) * mod;
		...
		SynthDef(\bLPat5Verb,{| in = 0, out = 0, amp=1.0, mix=0.5, decay=0.75, density=0.25, fund=60, randMin=0.8, randMax=1.2|
	...
	setDescription {
		description = "BLPat5: Audio rate granulator.";
	}
}