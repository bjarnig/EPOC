...
BLText2 : BGen
{
	...
	*new { |id=0, description, duration=10, control, outBus=0, values, sound, buf,load=1|
	...
	initEffect {
		...
		reverb = Bwrap.new(\bLText2Verb, [\inBus, effectBus, \outBus, outBus]);
		reverb.play;
	}

	*loadSynthDefs {

		SynthDef(\bLText2Grain, { |outBus=0, atk=4, sus=5, rel=6, amp=1, startPos=0.0, density=0.5, buf=0, rate=1.0, randSpeed=10, entropy=0.5, speed=0.01, ampMod=0.5,
