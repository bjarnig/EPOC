...
BSeqBorax : BSeq
{
	...
	*new { |id=0, description="BSeqBorax", duration=10, control, outBus=0, sounds, freqSeq, deltaSeq, durSeq, soundSeq, ampSeq, load=1|
	...
	init {|load=1|
		...
		reverb = Bwrap.new(\boraxReso, [\inBus, effectBus, \outBus, outBus]);
		reverb.play;

		distBus = Bus.audio(Server.local, 2);
		distortion = Bwrap.new(\boraxCrusher, [\inBus, distBus, \outBus, effectBus]);
		distortion.play;
	...
	*loadSynthDefs {
		...
		SynthDef(\boraxCrusher, {| inBus=0, outBus=0, samplerate=44100, bitsize=16, fxlevel=0.5, level=0.5, filter=1.0, filtamp=1.0|
		...
		SynthDef(\boraxReso, {| outBus = 0, inBus=2, amp=1, dryWet=0.01, roomsize=240, revtime=4.85, damping=0.21, inputbw=0.19,
