...
{ 	
	...
	*loadSynthDefs {
		...
		SynthDef(\opalVerb,{| Êd1 = 0.1, d2 = 0.15, d3 = 0.2, d4 = 0.25, d5 = 0.4,
...
Ê Ê Ê Ê	outB = (((c1 + c2 + c3 + c4 + c5) * 0.2) * mix) + (inB * (1 - mix));
	...
	var atk, sus, rel;	
		...
		if((this.waveform < 2) || (this.waveform > 4), {wrap = Bwrap.new(\opalSine, paramValues)}); 
		if(this.waveform == 2, {wrap = Bwrap.new(\opalImpulse, paramValues)}); 
		if(this.waveform == 3, {wrap = Bwrap.new(\opalLFTri, paramValues)}); 
	...
	{	
		wrap.set(\repAtk, Env.new([0.0, 0.001, 0.002, 0.008, 0.01],[0.25, 0.25, 0.25, 0.25]).at(control.density));
		wrap.set(\repSus, Env.new([0.001, 0.025, 0.04, 0,09, 0.15],[0.25, 0.25, 0.25, 0.25]).at(control.density));
		wrap.set(\repRel, Env.new([0.001, 0.08, 0.098, 0.11, 0.15],[0.25, 0.25, 0.25, 0.25]).at(control.density));
	});
	...
	setDescription {
		description = "BGenOpal: simple sine patterns";
	}
}