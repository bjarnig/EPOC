...
BGenChalk : BGen
{
	...
	*loadSynthDefs {
	...
	SynthDef(\chalkVerb, {|in=0, out=0, mix=0.01, d1 = 0.80, d2 = 0.82, d3 = 0.84, d4 = 0.86, d5 = 0.88,
		t1 = 2.1, t2 = 2.2, t3 = 2.3, t4 = 2.4, t5 = 2.5, f1 = 20000, f2 = 20000, f3 = 20000, f4 = 20000,
		f5 = 20000, amp=1.0, delayMult=0.01, decayMult=8.0, filtMult=0.6|

		var input, output, c1, c2, c3, c4, c5;
		input = In.ar(in, 2);

		c1 = LPF.ar(CombC.ar(input, 1, d1 * delayMult, t1 * decayMult), f1 * filtMult);
		c2 = LPF.ar(CombC.ar(input, 1, d2 * delayMult, t2 * decayMult), f2 * filtMult);
		c3 = LPF.ar(CombC.ar(input, 1, d3 * delayMult, t3 * decayMult), f3 * filtMult);
		c4 = LPF.ar(CombC.ar(input, 1, d4 * delayMult, t4 * decayMult), f4 * filtMult);
		c5 = LPF.ar(CombC.ar(input, 1, d5 * delayMult, t5 * decayMult), f5 * filtMult);

		output = (((c1 + c2 + c3 + c4 + c5) * 0.4) * mix) + (input * (1.0-mix));
