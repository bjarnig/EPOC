...
{ 	
	...
	*loadSynthDefs {
		...
		SynthDef(\pearlVerb,{| Êd1 = 0.1, d2 = 0.15, d3 = 0.2, d4 = 0.25, d5 = 0.4,
...
Ê Ê Ê Ê	outB = (((c1 + c2 + c3 + c4 + c5) * 0.2) * mix) + (inB * (1 - mix));
	...
	var atk, sus, rel;	
		...
		wrap = Bwrap.new(\pearl, paramValues);
	...
	{	
		wrap.set(\knum, Env.new([0.0, 6, 12, 18, 24],[0.25, 0.25, 0.25, 0.25]).at(control.density));
	...
	setDescription {
		description = "BGenPearl: Gendy through resonators";
	}
}