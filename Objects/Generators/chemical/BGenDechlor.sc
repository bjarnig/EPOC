...
BGenDechlor : BGen
{
	...
	*new { |id=0, description, duration=10, control, outBus=0, values, load=1|
	...
	initEffect {
		...
		reverb = Bwrap.new(\dechlorVerb, [\in, effectBus, \out, outBus]);
		reverb.play;
	}
	...
	*loadSynthDefs {
	...
	SynthDef(\dechlorVerb,{| d1 = 0.1, d2 = 0.15, d3 = 0.2, d4 = 0.25, d5 = 0.4,
		...
		c2 = LPF.ar(CombC.ar(inB, 1, d2 * delayMult, t2 * decayMult), f2 * filtMult) * 1.2;
		c3 = LPF.ar(CombC.ar(inB, 1, d3 * delayMult, t3 * decayMult), f3 * filtMult) * 1.2;
		c4 = LPF.ar(CombC.ar(inB, 1, d4 * delayMult, t4 * decayMult), f4 * filtMult) * 1.2;
		c5 = LPF.ar(CombC.ar(inB, 1, d5 * delayMult, t5 * decayMult), f5 * filtMult) * 1.2;
