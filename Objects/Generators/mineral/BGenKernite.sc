...
{ 	
	...
	*new { |id=0, description, duration=10, control, outBus=0, values, shaperBuf, load=1|
	...
	initEffect {
		...
		reverb = Bwrap.new(\kerniteVerb, [\inBus, effectBus, \outBus, outBus]);
		reverb.play;
	}
	
	*loadSynthDefs {
				
	SynthDef(\kernite, {arg outBus=0, atk=0.1, sus=400, rel=4, speed = 0.11, density = 0.8525, dryWet=0.94, pan=0.1, entropy=0.071, amp=0.74, pos=0.575, surface=0.1, 
	...
	{
		wrap.set(\amp, Env.new([0.0, 0.27, 0.75, 2.0, 4.0],[0.25, 0.25, 0.25, 0.25]).at(control.amplitude));
	});
