...
BGenCitrine : BGen
{
	...
	*new { |id=0, description, duration=10, control, outBus=0, values, sound, buf,load=1|
	...
	initEffect {
		...
		reverb = Bwrap.new(\citrineVerb, [\inBus, effectBus, \outBus, outBus]);
		reverb.play;
	}

	*loadSynthDefs {

	 	SynthDef(\citrineGrain,
		...
		fx = sig + LocalIn.ar(2);
	fx = (DelayC.ar(fx, maxDelay, delay) + LPF.ar(DelayC.ar(fx, maxDelay * 1.2, delay * 1.2), 200)) * 0.5;
