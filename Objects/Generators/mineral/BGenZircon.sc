...
{ 	
	...
	*loadSynthDefs {
	...
	if(updateAll == 1 || (control.surface != preControl.surface), 
	{
		...
		wrap.set(\lopVol, Env.new([0.9, 0.85, 0.5, 0.25, 0.1],[0.25, 0.25, 0.25, 0.25]).at(control.surface));
		wrap.set(\hipVol, Env.new([0.1, 0.25, 0.5, 0.85, 0.9],[0.25, 0.25, 0.25, 0.25]).at(control.surface));
	});
